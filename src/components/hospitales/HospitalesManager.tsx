...
import { HospitalesForm } from "./HospitalesForm";
import { HospitalesTabla } from "./HospitalesTabla";
import { ConfirmModal } from "../ui/ConfirmModal";

export const HospitalesManager = () => {
        ...
        try {
            if (hospitalEnEdicion) {
                const { error } = await hospitalRepo.updateHospital(
                    hospitalEnEdicion.id, nombre.trim(), provincia.trim(), codigoCentro.trim(), areaSalud.trim()
                );
                if (error) throw error;
                cancelarEdicion();
            } else {
                const { error } = await hospitalRepo.createHospital(
                    nombre.trim(), provincia.trim(), codigoCentro.trim(), areaSalud.trim()
                );
                if (error) {
                    if (error.code === '23505') throw new Error(`El código de centro '${codigoCentro}' ya existe.`);
                    throw error;
                }
    ...
    const confirmarBorrado = async () => {
        ...
        if (error) setErrorMsg("No se puede borrar porque tiene datos asociados. Desactívalo en su lugar.");
    ...
    return (
        <div className="space-y-6">
            ...
            <HospitalesForm 
                ...
                isEditing={!!hospitalEnEdicion} onCancelEdit={cancelarEdicion}
            />

            <HospitalesTabla 
                hospitales={hospitales} onToggleActivo={handleToggleActivo} 
                onDelete={solicitarBorrado} onEdit={iniciarEdicion} 
            />
