...
export const DashboardPage = () => {
    ...
    useEffect(() => {
        async function loadStats() {
            // Si es un gestor "fantasma" sin hospitales, ni siquiera preguntamos a la base de datos
            if (!isAdmin && misHospitales.length === 0) {
                setLoading(false);
                return;
            }

            // Le pasamos el array de hospitales y si es admin
            const { data: stast, error } = await statsRepo.getDashboardStats(misHospitales, isAdmin);
        ...
        loadStats();
    }, [misHospitales, isAdmin]); // Le decimos a React que recargue si cambian los hospitales


    // 2. EL MURO DE SEGURIDAD (Ahora va después del useEffect)
    if (!isAdmin && misHospitales.length === 0) {
        return (
    ...
    // 4. EL RENDERIZADO FINAL
    return (
        <div className="space-y-8 animate-fade-in">
            <div>
                <h1 className="text-3xl font-extrabold text-slate-900">
                    {isAdmin ? "Panel de Control Global" : "Panel de Control"}
                </h1>
